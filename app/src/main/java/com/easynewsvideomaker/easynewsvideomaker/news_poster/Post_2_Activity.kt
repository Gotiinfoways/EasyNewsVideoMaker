...
import com.easynewsvideomaker.easynewsvideomaker.databinding.ActivityPost2Binding
import com.easynewsvideomaker.easynewsvideomaker.databinding.DialogEditBinding
import yuku.ambilwarna.AmbilWarnaDialog


class Post_2_Activity : AppCompatActivity() {
    lateinit var binding: ActivityPost2Binding
    ...
    private fun initview() {
        ...
        binding.imgpick2.setOnClickListener {
            val Gallery = Intent(Intent.ACTION_PICK, MediaStore.Images.Media.INTERNAL_CONTENT_URI)
            Gallery_Launcher.launch(Gallery)
        }

        binding.imgcapture2.setOnClickListener {
            val intent = Intent(MediaStore.ACTION_IMAGE_CAPTURE)
            camera_Launcher.launch(intent)
        }

        binding.cdExploreBtn2.setOnClickListener {
        ...
        binding.txtheadingText.setOnClickListener {
            var text = binding.txtheadingText.text.toString()
            var textColor = binding.txtheadingText.currentTextColor
            editeDialog(text, textColor)
            ...
            dialogEditBinding.btnSubmit.setOnClickListener {
                ...
                binding.txtheadingText.text = dialogEditBinding.edtText.text.toString()
                binding.txtheadingText.setTextColor(colorText)
                Toast.makeText(this, "Your data is Change", Toast.LENGTH_SHORT).show()
                editeDialog.dismiss()
        ...
        binding.txtNewsChannel.setOnClickListener {
            var text = binding.txtNewsChannel.text.toString()
            var textColor = binding.txtNewsChannel.currentTextColor
            editeDialog(text, textColor)
            ...
            dialogEditBinding.btnSubmit.setOnClickListener {
                ...
                binding.txtNewsChannel.text = dialogEditBinding.edtText.text.toString()
                binding.txtNewsChannel.setTextColor(colorText)
                Toast.makeText(this, "Your data is Change", Toast.LENGTH_SHORT).show()
                editeDialog.dismiss()
        ...
        object : ActivityResultCallback<ActivityResult?> {
            override fun onActivityResult(result: ActivityResult?) {
                if (result!!.resultCode == RESULT_OK) {
                    ...
                    Glide.with(this@Post_2_Activity)
                        .load(b)
                        .transform(CenterCrop())
                        .into(binding.imgpost2)
                }
